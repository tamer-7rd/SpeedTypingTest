...
def test_leaderboard_manager_duplicate_detection(temp_leaderboard_file):
    ...
    leaderboard_manager.add_to_leaderboard(
        "User1", "test@test.com", "Country1", 50.0, 95.0, 47.5)
    leaderboard, position = leaderboard_manager.add_to_leaderboard(
        "User1", "test@test.com", "Country1", 50.0, 95.0, 47.5)
    assert len(leaderboard) == 1
    assert position == 1
...
def test_leaderboard_manager_sorting(temp_leaderboard_file):
    ...
    leaderboard_manager.add_to_leaderboard(
        "User1", "test1@test.com", "Country1", 50.0, 80.0, 40.0)
    leaderboard_manager.add_to_leaderboard(
        "User2", "test2@test.com", "Country2", 60.0, 90.0, 54.0)
    leaderboard_manager.add_to_leaderboard(
        "User3", "test3@test.com", "Country3", 40.0, 95.0, 38.0)
...
def test_leaderboard_manager_max_entries(temp_leaderboard_file):
    ...
    for i in range(55):
        leaderboard_manager.add_to_leaderboard(
            f"User{i}", f"test{i}@test.com", f"Country{i}", 50.0, 80.0, 40.0)
